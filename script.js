...
function paxTableHTML(pax) {
  ...
  pax.forEach((p) => {
    ...
    if (carrier) {
      /* أسماء المسافرين بخط عادي (Regular) — العناوين فقط Bold كما في المرجع */
      rows.push(`<tr><td class="pv-n">${escapeHtml(p.name)}</td><td class="pv-s"><img class="ic-inf" src="assets/tk/infant.png" alt="" draggable="false"></td></tr>
      ...
      rows.push(`<tr><td class="pv-n">${escapeHtml(p.name)}${paxTag(p)}</td><td class="pv-s"></td></tr>
